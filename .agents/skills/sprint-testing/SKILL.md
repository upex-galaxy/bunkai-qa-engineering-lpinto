...
- **DISPATCH, NEVER INLINE (hard gate):** each stage runs via its `Role agent` (`subagent_type`). `Session Start` + `Stage 1` → `qa-plan`; `Stage 2` → `qa-bulk` (mechanical) or `qa-plan` (exploratory reasoning); `Stage 3` → `qa-write`. The orchestrator MUST NOT execute stage work inline — no writing ATP/ATR to Jira fields, no `dbhub`/`curl`/Playwright exploration, no filing bugs, no posting QA comments, no transitions — even when the context is already loaded. The "most efficient path" carve-out applies to NON-stage work only (memory reads/writes, task tracking, planning, asking the user, sprint-file bookkeeping). If you catch yourself about to run a stage action inline, STOP and dispatch the role agent.
...
| Stage                                              | Pattern    | Role agent (`subagent_type`)                       | Subagent role                                                                                                                                                                  |
|----------------------------------------------------|------------|----------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Session Start (per-ticket)                         | Single     | `qa-plan`                                          | fetch ticket from issue tracker, load `.context/`, create the PBI folder + `context.md` and the session dir + `test-session-memory.md`, return ticket summary + AC list |
| Stage 1 — Planning (ATP + draft TCs + risk triage) | Sequential | `qa-plan`                                          | produce the ATP artifact + risk score + draft TC outlines; bug tickets get the veto + triage decision tree applied                                |
| Stage 2 — Execution (smoke + UI/API/DB exploration)| Sequential | `qa-bulk` (mechanical CLI) / `qa-plan` (exploratory reasoning) | smoke pass first, then triforce (UI/API/DB) exploration; capture evidence under the PBI folder; surface BUG_FOUND if applicable                  |
| Stage 3 — Reporting (ATR + QA comment + transition)| Sequential | `qa-write`                                         | fill the ATR, post the QA comment, transition the issue, file bug reports if any                                                                 |

> **Always dispatch by the `Role agent`, never `general`/`explore`.** The `Role agent` column above is authoritative per `AGENTS.md` §"Orchestration Mode". Supply it as `subagent_type` on every dispatch. Role→model binding lives in `.context/ai-model-strategy.md` and the harness agent configs (`.opencode/agents/`, `.codex/agents/`, `.claude/agents/`). **Stage 2 split rule**: when the execution pass is dominated by mechanical CLI operations (browser clicks, `curl`, MCP lookups, bulk evidence capture) → dispatch `qa-bulk`; when it requires exploratory test judgment (bug triage, severity calls, "does this behavior violate the AC?" reasoning) → dispatch `qa-plan`. Either choice runs the SAME Stage 2 briefing (Briefing 3) verbatim — only the `subagent_type` differs.
...
2. **ATP item FROM the field (find-or-create).** Pre-sprint the ATP lives ONLY in `{{jira.acceptance_test_plan}}` — the shift-left pass is field-first and does NOT create the item. Find-or-create the Test Plan issue `ATP: {STORY-KEY}: {story title}` (parent: **QA Master Test Plan** epic) and seed its description from the field content; if the field is empty (no shift-left pass), author the ATP normally and write both the item and the field. **Then link ATP→Story via the `test` slug** (`{{jira.link_types.test}}`, Story `is tested by` ATP) — administrative traceability only, ZERO coverage (never skip this link: it is what binds the Plan item to the Story in Jira's link graph).
...
4. **ATR always with environment.** Create the ATR Execution (`ATR: {STORY-KEY}: Story Testing`, parent: **QA Test Artifacts**) ALWAYS carrying the Test Environment resolved from `active_env` in `.agents/project.yaml` (or the session env switch). **Then link ATR→Story via the `test` slug** (`{{jira.link_types.test}}`, Story `is tested by` ATR) — administrative traceability only, ZERO coverage. **No ATR without environment** — hard gate: `agentic-qa-core/references/stage-gates.md` §Stage 1.
...
9. **Traceability check after Stage 1** (Modality jira-xray): run `[TMS_TOOL] trace {TICKET}` and verify the Set-first model — the **coverage backbone is the ATS**: `ATS: {US_ID}: {story title}` linked to the Story via the `test` slug ("is tested by") and holding ALL the Story's TCs (Xray-internal membership, never issue links in this modality). **Story↔ATP and Story↔ATR** links exist as administrative traceability — they contribute ZERO coverage (live-verified) but MUST still be created in Stage 1 (ATP→Story and ATR→Story via the `test` slug) so the Story's Jira link graph is complete. The ATP's and the ATR's test lists are DERIVED from the ATS membership. **Individual TCs are NOT linked directly to the Story** (last-resort only, for instances with no Test Set work type). So verify: Story↔ATS (`test` slug) + Story↔ATP + Story↔ATR (all three `test` links, direction-verified) + ATS membership complete + Plan/Exec lists matching the ATS. Full doctrine: `agentic-qa-core/references/traceability-linking.md` + `test-documentation/references/tms-architecture.md`. Bugs: the repro Test links Bug↔Test via the `test` slug at fix-verification time; before that, traceability "gaps" for missing TCs are expected and OK.
...
- [ ] Stage 1 artifacts created with full traceability, verified via `[TMS_TOOL] trace` — jira-xray: Set-first order honored (ATP item from the field · ATS with ALL TCs linked to the Story via the `test` slug · **ATP→Story and ATR→Story `test` links created** · Plan/Exec lists derived from the ATS membership)
