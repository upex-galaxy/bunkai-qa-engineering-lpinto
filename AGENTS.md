...
**MODEL ROUTING (subagent → role → model)**: dispatch a QA subagent by its ROLE agent, never the generic `general`/`explore`. Role agents are bound to models in harness config (`.opencode/agents/`, `.codex/agents/`, `.claude/agents/`). The orchestrator maps the task type to its role:
...
DeepSeek peak hours (`01:00–04:00` + `06:00–10:00` UTC Mon–Fri) → swap to `opencode-go/glm-5.3`. Free models are mechanical-only (mini-eval: Nemotron Ultra Free 8/12 vs Pro 12/12 — fabricates under ambiguity). Full table + per-harness IDs: `.context/ai-model-strategy.md`.
