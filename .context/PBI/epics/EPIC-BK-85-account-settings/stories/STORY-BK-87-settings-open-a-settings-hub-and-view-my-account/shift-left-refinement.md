# Shift-Left Refinement: BK-87 — Settings | Open a settings hub and view my account
...
- **Frontend**: Next.js App Router — new route under `app/(app)/settings/` (greenfield, no existing settings pages). Needs:
  - Settings layout (tab-based or sidebar sub-nav)
  - Account section showing identity (email, name from Supabase Auth), role in current workspace, sign-out action
  - Global nav integration: link to `/settings` in Topbar (user menu or gear icon)
- **Backend**: Minimal — identity is already available client-side via `AuthProvider` (`useAuth() → user.email, user.user_metadata`). Workspace membership queried from `workspace_members` (Supabase). No new API endpoints required for read operations.
...
- **Then**:
  - UI: Shows user email (`sara@example.com`) as the primary identity label
  - UI: Shows user display name (`Sara Iglesias`) if available in `user_metadata`
  - UI: Shows the user's role in the CURRENT workspace (e.g., "Owner", "Admin", "Member", "Viewer")
...
- **When**: The user navigates to Settings → Account section
- **Then**:
  - UI: Shows a list/section titled "Workspaces" or "My Workspaces"
  - UI: Lists each workspace: name + role (e.g., "Bunkai Core — Owner", "QA Sandbox — Member")
  - UI: The current workspace is visually indicated (e.g., "(current)" badge or highlighted row)
...
3. **How should `user_metadata` be populated?** — Supabase Auth `user_metadata` is empty by default. Is there a plan to let users set their display name, or is email always the primary identity label?
