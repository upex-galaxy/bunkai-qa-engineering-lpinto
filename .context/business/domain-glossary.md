...
> Generated: 2026-08-09 (refreshed 2026-08-27)
> Mirror of: `../upex-bunkai-tms/.context/business/domain-glossary.md` (canonical, last full code-sync 2026-08-12 against `origin/staging@4924f48`; local refresh 2026-08-27 against migrations 0071–0075)
...
| **Bug Detail (BugDetailSchema)** | (BK-337, BK-466, migration 0070) The expanded Bug JSON returned by `GET /api/v1/bugs/{id}` and by the create/assign/status RPCs. Shape includes: all `bugs` columns plus `origin` (provenance: `run_id`, step position, ATC title/layer — nullable when the bug was filed outside a run context) and `module.archived_at` (null = active, non-null = archived module). `bunkai_bug_json` is SECURITY INVOKER, RLS-scoped; null return = not found or hidden (non-disclosing 404). Evidence URLs (`evidence_url`, `evidence_urls`) are restricted to http/https schemes (BK-466) — data URLs and other schemes are rejected. |
...
| **Command Palette** | The keyboard-driven overlay (⌘K) for searching and jumping across the workspace from any screen. Spans the six entity types with shipped routes: ATCs, Tests, Projects, Modules, Bugs, Runs. Implemented via `bunkai_search_workspace` RPC (SECURITY INVOKER, migration 0071) with GIN expression indexes on each entity's title/name field. Returns a union across 6 entity types with a 5-per-group cap. It is an *overlay on the app shell*, not a screen — it has no route and no mockup of its own. Say "command palette", never "search bar" (which names the ATC Library's in-page filter) and never "omnibox". |
...
| **Abandoned Run** | A `running` Run whose executor has disconnected or stalled. Detected by the Abandoned-Run Sweep: any Run still in `running` status for longer than 15 minutes is auto-transitioned to `aborted` with reason "abandoned" (via the sweep's `bunkai_close_abandoned_runs` RPC). The Run's `aborted` terminal status is the same as a manual abort — the sweep does not invent a new outcome. Distinct from a *finished* Run with `failed` verdict: an abandoned Run's step statuses may be partially `pending` rather than all resolved. |
| **Abandoned-Run Sweep** | (BK-269, migration 0075) A pg_cron job (`bunkai_close_abandoned_runs`) that runs every 5 minutes, identifies `running` Runs older than 15 minutes, and transitions them to `aborted` with reason "abandoned". SECURITY DEFINER — no HTTP route; callable directly via SQL for testing. Available in pg_cron but not yet installed on the live instance. Part of the Automatic Processes inventory (§4). |

Post-MVP entities (epics BK-201 / BK-208 / BK-210 / BK-221 / BK-224), added 2026-07-11:

| Entity | Definition |
| --- | --- |
| **Test Plan** | Named grouping of Tests inside a Project for a goal, cycle, or release. Membership is curated — a Test may belong to multiple plans. Progress is derived from members' latest run outcomes. Closing a plan captures an outcome summary and makes it read-only. **Not the ATP** (Acceptance Test Plan, §1) — the ATP is the per-US QA documentation artifact; a Test Plan is the TMS grouping entity. Never shorten either to bare "plan" (see §4). Name: 1–100 chars, case-insensitive unique per project. Description (≤500 chars) and goal (≤100 chars) are optional. Whitespace normalization: collapse ASCII whitespace → trim; U+00A0 (NBSP) is preserved (BK-591, migration 0074). |
...
The previous version of this file (generated 2026-06-06) contained the **incorrect ATC expansion "Atomic Test Component"**, an invented `draft/active/archived` status lifecycle that does not exist in the schema, and fabricated endpoints (`POST /api/v1/projects`, `/user-stories`, `/anchoring`). All of that is superseded by this mirror of the target's canonical glossary (2026-08-09, staging state). When refreshing, re-mirror from `../upex-bunkai-tms/.context/business/domain-glossary.md` — this file is a mirror, not an independent source. Refreshed 2026-08-27 to carry: BK-398 ⌘K search (Command Palette functional, GIN indexes, search RPC), BK-229 workspace billing overview, BK-202 test plans (NBSP-aware whitespace normalization BK-591), BK-269 abandoned-run sweep (pg_cron), BK-337 bug detail read (origin provenance), BK-466 evidence URL guard (http/https only).
